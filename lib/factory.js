...
function factory( options ) {
	...
	if ( arguments.length ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0ox47,Gz', options ) );
		}
		...
		if ( hasOwnProp( options, 'state' ) ) {
			...
			if ( opts.state === void 0 ) {
				throw new TypeError( format( '0ox7Y,NT', 'state', opts.state ) );
			}
		} else if ( hasOwnProp( options, 'seed' ) ) {
			opts.seed = options.seed;
			if ( opts.seed === void 0 ) {
				throw new TypeError( format( '0ox7Y,NT', 'seed', opts.seed ) );
			}
		}
		if ( hasOwnProp( options, 'copy' ) ) {
			opts.copy = options.copy;
			if ( !isBoolean( opts.copy ) ) {
				throw new TypeError( format( '0ox2o,GE', 'copy', opts.copy ) );
			}
		}
	}
	prng = PRNGS[ opts.name ];
	if ( prng === void 0 ) {
		throw new Error( format( '0ox7Z,NU', opts.name ) );
	}
	if ( opts.state === void 0 ) {
		if ( opts.seed === void 0 ) {
			rand = prng.factory();
